...
export class StudentNextTimetableComponent implements OnInit, OnDestroy {
...
isBtnLoading: any;
  ...
  ngOnInit(): void {
    this.getTimetableData = JSON.parse(localStorage.getItem('studentTimeTable')!);
    let count = 1;
    this.getTimetableData.studentTimetable.map((i:any)=>{
      if (!i.selectBreak) {
  ...
  initNewForm(obj: any = {}) {
    ...
    this.addMoreRows(this.getTimetableData.studentTimetable.length); 
  }
  ...
  addMoreRows(numberOfRows: any) {
    for (let index = 0; index < numberOfRows; index++) {
      const item = this.getTimetableData.studentTimetable[index];
      if (item.selectBreak) {
