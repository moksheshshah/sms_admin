...
export interface studentsReportComponent{
  checkbox:any;
  photo:any;
  adms_no:any;
  roll_no:any;
...
const STDATTENDANCE_DATA:studentsReportComponent[]=[
...
export class StudentsReportComponent {
  ...
  selection = new SelectionModel<studentsReportComponent>(true , []);
  ...
  editStudentReport(event:any,resData:any){
  ...
  deleteStudentReport(element:any){
    this.isTableLoading = true;
    const dialogRef = this._dialog.open(CommonModalComponent,{
      width:'410px',
      height:'fit-content',
